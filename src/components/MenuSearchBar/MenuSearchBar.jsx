...
const MenuSearchBar = () => {
  ...
  // Get first three menu item names
  const menuNames = menuItems.slice(0, 3).map((item) => item.name);

  useEffect(() => {
    if (menuNames.length === 0) return;

    const handleTyping = () => {
      const current = menuNames[typingIndex];
      ...
      if (isDeleting && currentText === "") {
        ...
        setTypingIndex((prev) => (prev + 1) % menuNames.length);
        return;
      }
    };
  ...
  }, [currentText, isDeleting, typingIndex, menuNames, typingSpeed]);
